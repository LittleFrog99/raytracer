...
void MultiJittered::generateSamples() {
    ...
    for (int p = 0; p < numSets; p++) // initial patterns
        for (int i = 0; i < num; i++)
            for (int j = 0; j < num; j++) {
                double x = (i * num + j) * subcell_width + rndm.randomDouble() * subcell_width;
                double y = (j * num + i) * subcell_width + rndm.randomDouble() * subcell_width;
    ...
    for (int p = 0; p < numSets; p++) // shuffle x coordinates
        for (int i = 0; i < num; i++)
            for (int j = 0; j < num; j++) {
                int k = rndm.randomInteger(j, num - 1);

...
public:
    ...
    virtual vec3 sampleF(Shade &shade, dvec3 &in, dvec3 &out) = 0;
    virtual vec3 sampleF(Shade &shade, dvec3 &in, dvec3 &out, float &reflect) = 0;
    virtual vec3 calcReflectance(Shade &shade, dvec3 &out) = 0;

    inline void setSampler(Sampler *sampler_ptr) { samplerP = sampler_ptr; }
